...
class Boy:
    def __init__(self):
        ...
        self.x = server.background.w / 2 # 0부터 sever.background.w까지 0~1836
        self.y = server.background.h / 2 # 0부터 sever.background.h까지 0~1108
    ...
    def draw(self):
        # x, y는  월드 좌표계인데 sx, sy 화면 좌표계로 변환
    ...
    def get_bb(self):
        sx = self.x - server.background.window_left
        sy = self.y - server.background.window_bottom
        return sx - 25, sy - 50, sx + 25, sy + 50
